...
${NUODB_MIGRATOR_HOME}/bin/nuodb-migrator schema --source.driver=${SOURCE_DRIVER} --source.url=${SOURCE_URL}   --source.username=${SOURCE_USERNAME} --source.password=${SOURCE_PASSWORD}  --output.path=${WORK_DIR}/dump/dump.sql
echo "---------------------------------------------------------------------------------------"
echo "Running dump command"
${NUODB_MIGRATOR_HOME}/bin/nuodb-migrator dump --time.zone=EST --source.driver=${SOURCE_DRIVER} --source.url=${SOURCE_URL}   --source.username=${SOURCE_USERNAME} --source.password=${SOURCE_PASSWORD}   --output.type=${FORMAT} --output.path=${WORK_DIR}/dump/dump.cat
echo "========================================================================================="
echo "Loading schema"
