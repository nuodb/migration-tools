...
/**
 * <p>A random-access buffer that resizes itself. This buffer differentiates from {@link java.io.ByteArrayOutputStream}
 * in the following points:</p> <ul> <li>It allows specifying the byte order.</li> <li>It assigns several internal
 * buffers instead of one and therefore saves garbage collection cycles.</li> <li>It is able to flush some of its
 * internal buffers to an output stream or to a writable channel.</li> </ul> <p>The buffer has an initial size. This is
 * also the size of each internal buffer, so if a new buffer has to be allocated it will take exactly that many
 * bytes.</p> <p>By calling {@link #flushTo(OutputStream)} or {@link #flushTo(WritableByteChannel)} some of this
 * buffer's internal buffers are flushed and then deallocated. The buffer maintains an internal counter for all flushed
 * buffers. This allows the {@link #writeTo(OutputStream)} and {@link #writeTo(WritableByteChannel)} methods to only
 * write non-flushed buffers. So, this class can be used for streaming by flushing internal buffers from time to time
 * and at the end writing the rest:
 * <pre>
 * ...
 * buf.flushTo(out);
 * ...
 ...
 * ...
 * buf.writeTo(out);</pre>
 * </p> <p>If flushing is never used a single call to one of the <code>writeTo</code> methods is enough to write the
 * whole buffer.</p> <p>Once the buffer has been written to an output stream or channel, putting elements into it is not
 * possible anymore and will lead to an {@link java.lang.IndexOutOfBoundsException}.</p>
 *
...
public class DynamicOutputBuffer {
    ...
    /**
     * The position of the first byte that has not been already flushed. Any attempt to put something into the buffer at
     * a position before this first byte is invalid and causes a {@link IndexOutOfBoundsException} to be thrown.
     */
    ...
    /**
     * The character set used in {@link #putUTF8(String)}. Will be created lazily in {@link #getUTF8Charset()}
     */
    private Charset _utf8;

    /**
     * The encoder used in {@link #putUTF8(String)}. Will be created lazily in {@link #getUTF8Encoder()}
     */
    ...
    /**
     * Creates a dynamic buffer with BIG_ENDIAN byte order and a default initial buffer size of {@link
    ...
    /**
     * Creates a dynamic buffer with the given byte order and a default initial buffer size of {@link
     * #DEFAULT_BUFFER_SIZE} bytes.
     *
    ...
    /**
     * Sets the number of buffers to save for reuse after they have been invalidated by {@link #flushTo(OutputStream)}
     * or {@link #flushTo(WritableByteChannel)}. Invalidated buffers will be saved in an internal queue. When the buffer
     * needs a new internal buffer, it first attempts to reuse an existing one before allocating a new one.
     *
    ...
    /**
     * Gets the buffer that holds the byte at the given absolute position. Automatically adds new internal buffers if
     * the position lies outside the current range of all internal buffers.
     *
    ...
    /**
     * Adapts the buffer size so it is at least equal to the given number of bytes. This method does not add new
     * internal buffers.
     *
    ...
    /**
     * Puts a 64-bit integer into the buffer at the current write position and increases the write position
     * accordingly.
     *
    ...
    /**
     * Puts a 32-bit floating point number into the buffer at the current write position and increases the write
    ...
    /**
     * Puts a 64-bit floating point number into the buffer at the current write position and increases the write
     * position accordingly.
     *
    ...
    /**
     * Puts a character sequence into the buffer at the current write position and increases the write position
     * accordingly.
     *
    ...
    /**
     * Puts the given string as UTF-8 into the buffer at the given position. This method does not increase the write
     * position.
     *
     ...
     * #flushTo(WritableByteChannel)} for further information.
     *
    ...
    /**
     * Tries to copy as much bytes as possible from this buffer to the given channel. This method always copies whole
     * internal buffers and deallocates them afterwards. It does not deallocate the buffer the write position is
     * currently pointing to nor does it deallocate buffers following the write position. The method increases an
     * internal pointer so consecutive calls also copy consecutive bytes.
     *
    ...
    /**
     * Writes all non-flushed internal buffers to the given output stream. If {@link #flushTo(OutputStream)} has not
     * been called before, this method writes the whole buffer to the output stream.
     *
     * @param out the output stream to write to
    ...
    /**
     * Writes all non-flushed internal buffers to the given channel. If {@link #flushTo(WritableByteChannel)} has not
     * been called before, this method writes the whole buffer to the channel.
     *
     * @param out the channel to write to
     * @throws IOException if the buffer could not be written
     */
