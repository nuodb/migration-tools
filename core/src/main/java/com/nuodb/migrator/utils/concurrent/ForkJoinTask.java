 ...
 * <p/>
 * <p>
 * A "main" {@code ForkJoinTask} begins execution when submitted to a
 * {@link ForkJoinPool}. Once started, it will usually in turn start other
 * subtasks. As indicated by the name of this class, many programs using {@code
 * ForkJoinTask} employ only methods {@link #fork} and {@link #join}, or
 * derivatives such as {@link #invokeAll(ForkJoinTask...) invokeAll}. However,
 * this class also provides a number of other methods that can come into play in
 * advanced usages, as well as extension mechanics that allow support of new
 ...
 * are only partially statically enforceable) reflecting their intended use as
 ...
 * Phasers that are advertised to cooperate with fork/join scheduling. Tasks
 * should also not perform blocking IO, and should ideally access variables that
 * are completely independent of those accessed by other running tasks. Minor
 * breaches of these restrictions, for example using shared output streams, may
 * be tolerable in practice, but frequent use may result in poor performance,
 * and the potential to indefinitely stall if the number of threads not waiting
 * for IO or other external synchronization becomes exhausted. This usage
 * restriction is in part enforced by not permitting checked exceptions such as
 * {@code
 * IOExceptions} to be thrown. However, computations may still encounter
 * unchecked exceptions, that are rethrown to callers attempting to join them.
 * These exceptions may additionally include {@link RejectedExecutionException}
 * stemming from internal resource exhaustion, such as failure to allocate
 * internal task queues. Rethrown exceptions behave in the same way as regular
 * exceptions, but, when possible, contain stack traces (as displayed for
 * example using {@code ex.printStackTrace()}) of both the thread that initiated
 * the computation as well as the thread actually encountering the exception;
 ...
 * fork/join processing, typically {@link RecursiveAction} for computations that
 * do not return results, or {@link RecursiveTask} for those that do. Normally,
 * a concrete ForkJoinTask subclass declares fields comprising its parameters,
 * established in a constructor, and then defines a {@code compute} method that
 * somehow uses the control methods supplied by this base class. While these
 * methods have {@code public} access (to allow instances of different task
 * subclasses to call each other's methods), some of them may only be called
 * from within other ForkJoinTasks (as may be determined using method
 * {@link #inForkJoinPool}). Attempts to invoke them in other contexts result in
 * exceptions or errors, possibly including {@code ClassCastException}.
 * <p/>
 * <p>
...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
     ...
     * ForkJoinWorkerThread and ForkJoinPool. The methods of this class are
     * more-or-less layered into (1) basic status maintenance (2) execution and
     * awaiting completion (3) user-level methods that additionally report
     * results. This is sometimes hard to see because this file orders exported
     ...
     * which status holds value NORMAL, CANCELLED, or EXCEPTIONAL. Tasks
     ...
     * wait/notify to take advantage of "monitor inflation" in JVMs that we
     * would otherwise need to emulate to avoid adding further per-task
     * bookkeeping overhead. We want these monitors to be "fat", i.e., not use
     * biasing or thin-lock techniques, so use some odd coding idioms that tend
     * to avoid them.
     */
    ...
    /**
     * Marks completion and wakes up threads waiting to join this task, also
     * clearing signal request bits.
     *
     * @param completion
    ...
    /**
     * Tries to block a worker thread until completed or timed out. Uses
     * Object.wait time argument conventions. May fail on contention or
     * interrupt.
     *
     * @param millis
     *            if > 0, wait time.
    ...
    /**
     * Report the result of invoke or join; called only upon non-normal return
     * of internal versions.
     */
     ...
     * completed and been reinitialized. Subsequent modifications to the state
     * of this task or any data it operates on are not necessarily consistently
     * observable by any thread other than the one executing it unless preceded
     * by a call to {@link #join} or related methods, or a call to
     * {@link #isDone} returning {@code true}.
     * <p/>
     * <p>
     * This method may be invoked only from within {@code ForkJoinPool}
     * computations (as may be determined using method {@link #inForkJoinPool}).
     * Attempts to invoke in other contexts result in exceptions or errors,
     * possibly including {@code ClassCastException}.
     *
     ...
     * {@code CancellationException} if cancelled, or {@code
     ...
     * succeed if this task is the most recently forked task by the current
     * thread, and has not commenced executing in another thread. This method
     * may be useful when arranging alternative local processing of tasks that
     * could have been, but were not, stolen.
     * <p/>
     * <p>
     * This method may be invoked only from within {@code ForkJoinPool}
     * computations (as may be determined using method {@link #inForkJoinPool}).
     ...
     * steal them. This value may be useful for heuristic decisions about
     * whether to fork other tasks. In many usages of ForkJoinTasks, at steady
     * state, each worker should aim to maintain a small constant surplus (for
     * example, 3) of tasks, and to process computations locally if this
     ...
     * task completed abnormally, or {@code
     * null} if this task is not known to have been completed. This method is
     * designed to aid debugging, as well as to support extensions. Its use in
    ...
    /**
     * Immediately performs the base action of this task. This method is
     * designed to support extensions, and should not in general be called
     * otherwise. The return value controls whether this task is considered to
     * be done normally. It may return false in asynchronous actions that
     * require explicit invocations of {@link #complete} to become joinable. It
     * may also throw an (unchecked) exception to indicate abnormal exit.
     *
     ...
     * current thread but not yet executed. This method is designed primarily to
    ...
    /**
     * Unschedules and returns, without executing, the next task queued by the
     * current thread but not yet executed, if one is available, or if not
     * available, a task that was forked by some other thread, if available.
     * Availability may be transient, so a {@code null} result does not
     * necessarily imply quiescence of the pool this task is operating in. This
     * method is designed primarily to support extensions, and is unlikely to be
     * useful otherwise.
     * <p/>
     * <p>
     * This method may be invoked only from within {@code ForkJoinPool}
